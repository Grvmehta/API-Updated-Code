...
public class Getagents
{
	...
	public String extractingAgentId()
	{
		...
		JSONObject jsonResult = new JSONObject(localresponseJSONString);
		String agentId;
		JSONObject hotels = jsonResult.getJSONObject("hotelogix").getJSONObject("response").getJSONArray("hotels").getJSONObject(0);
		JSONObject agents=hotels.getJSONArray("agents").getJSONObject(0);
		agentId=agents.getString("id");
